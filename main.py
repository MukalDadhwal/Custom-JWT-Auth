...
async def custom_exception_handler(request: Request, exc: MyCustomException) -> JSONResponse:
    return JSONResponse(
...
database: dict[str, User] = {
    "1234": User(
        user_id="1234", username="abra", password="xyz", email="bla@gmail.com"
    ),
}
...
def create_jwt_token(user: User) -> str:
    ...
    curr_epoch = int(datetime.datetime.utcnow().timestamp())
...
def validate_jwt_token(received_token: str) -> Union[str, None]:
    try:
        ...
        datetime_exp = datetime.datetime.utcfromtimestamp(issue_timestamp)
        datetime_curr = datetime.datetime.utcfromtimestamp(curr_timestamp)
